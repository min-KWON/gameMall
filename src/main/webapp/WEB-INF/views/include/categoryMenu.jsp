...
<div id="categoryMenu">
  <ul class="nav justify-content-center" id="mainCategory">
    <c:forEach items="${cateList }" var="categoryVO">
      <li class="nav-item">
        <a class="nav-item" href="${categoryVO.cat_code }"><c:out value="${categoryVO.cat_name }" /></a>
      </li>
    </c:forEach>
  </ul>
</div>
  ...
  $("ul.nav li.nav-item a").on("mouseover", function(e) {
    ...
    $.getJSON(url, function(subCategory) {
      ...
      for(let i=0; i<subCategory.length; i++) {
        ...
        subCategoryStr +=  '<a class=""nav-item"" href="' + subCategory[i].cat_code  + '">' + subCategory[i].cat_name + '</a>';
        subCategoryStr +=  '</li>';
      }
      subCategoryStr += '</ul>';
